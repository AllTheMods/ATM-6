...
onEvent('recipes', e => {
  ...
  function tinkerMelting(material, type, typeValues, temperature, time, byproduct) {
      ...
      e.custom(type != 'ores' ? recipe : Object.assign({
          byproducts: byproduct
      }, recipe)).id(`kubejs:melting/${type}/${material}`)
  }
  ...
  function tinkerAlloys(entries) {
      entries.forEach(([output, outputAmount, temperature, inputs]) => {
          e.remove({
              id: `tconstruct:smeltery/alloys/molten_${output}`
          })
          e.custom({
      ...
      entries.forEach(([material, temperature, time, types, byproduct]) => {
          ...
          meltingTypes.forEach(meltingType => {
              ...
              for (let type in meltingTypeValues)
                  if (types.includes(type)) tinkerMelting(material, type, meltingTypeValues[type], temperature, time, byproduct)
          })
          meltingParts.forEach(meltingPart => e.remove({
  ...
  function tinkerAlloyMelting(entries) {
      entries.forEach(([input, outputItem, outputBlock, outputNugget]) => {
          e.remove({
              id: `tconstruct:smeltery/casting/metal/${input}/block`
          })
  ...
  unifyTinkers([
      ...
      [
          ...
          ['storage_blocks', 'dusts', 'gears', 'ingots', 'nuggets', 'ores', 'plates', 'sheetmetals'],
          [{
              fluid: 'alltheores:molten_platinum',
              amount: 16
          }]
      ],
      [
  ...
  e.remove({
      id: 'tconstruct:smeltery/melting/copper/smeltery_controller'
  })
  e.remove({
      id: 'tconstruct:smeltery/melting/copper/smeltery_io'
  })
  tinkerBlockMelting([
      [{
  ...
  function unifyExtraStorageDisks(entries) {
      ...
      entries.forEach(size => {
          storageTypes.forEach(([tagCategory, recipeCategory, disk, storage]) => {
              ...
              e.remove({
                  id: `extrastorage:${recipeCategory}/${storage}_${size}k`
              })
              e.remove({
                  id: `extrastorage:${recipeCategory}/${storage}_${size * 64}k_fluid`
              })
          })
          e.remove({
              id: `extrastorage:disk/shapeless/disk_${size}k`
          })
          e.remove({
              id: `extrastorage:disk/shapeless/disk_${size * 64}k_fluid`
          })
      })
  }
  ...
  function createHoneyMixing(entries) {
      entries.forEach(([output, amount, honey, inputs, id]) => {
          ...
          inputs.forEach(([input, type]) => honeyMixingInputs.push(type == 0 ? {
              item: input
          } : {
              tag: input
          }))
